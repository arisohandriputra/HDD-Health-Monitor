...
/* ============================================================================
 *  Save Report  - exports the full drive health snapshot to a plain-text
 *  .txt file at <Documents>\HDDH_Reports\HDDH_Report_YYYYMMDD_HHMMSS.txt
 *  Triggered by the "Save Report" button on the main window or by the
 *  File -> Save Report... menu entry (Ctrl+R).
 * ============================================================================
 */
static void Report_WriteIndent(HANDLE hFile, const char* szLine)
{
    DWORD dwWritten = 0;
...
static void DoSaveReport(HWND hWnd)
{
    /* Resolve output directory: <Documents>\HDDH_Reports */
    char szDocDir[MAX_PATH] = "";
    if (!SHGetSpecialFolderPathA(NULL, szDocDir, CSIDL_PERSONAL, TRUE)) {
        GetModuleFileNameA(NULL, szDocDir, MAX_PATH);
        char* p = strrchr(szDocDir, '\\');
    ...
    char szFile[MAX_PATH];
    _snprintf(szFile, sizeof(szFile),
        "%s\\HDDH_Report_%04d%02d%02d_%02d%02d%02d.txt",
        szOutDir,
        st.wYear, st.wMonth, st.wDay,
        st.wHour, st.wMinute, st.wSecond);
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        ...
        _snprintf(szMsg, sizeof(szMsg),
            ...
            szFile, (unsigned long)GetLastError());
        MessageBoxA(hWnd, szMsg, "HDDHealth Monitor - Save Report Error",
            MB_OK | MB_ICONERROR);
        return;
    }
    ...
    #define RPT_LINE(...) do { \
        ...
        Report_WriteIndent(hFile, _szLine); \
    } while(0)
    ...
    RPT_LINE("================================================================");
    RPT_LINE("");
    RPT_LINE("Drives detected: %d", g_nDriveCount);
    RPT_LINE("");
    ...
    for (i = 0; i < g_nDriveCount && i < MAX_DRIVES; i++) {
        ...
        RPT_LINE("Model           : %s",
            strlen(pInfo->szModel) ? pInfo->szModel : "(unknown)");
        RPT_LINE("Serial Number   : %s",
        ...
        RPT_LINE("Power-On Hours  : %lu", (unsigned long)pInfo->dwPowerOnHours);
        RPT_LINE("Power Cycles    : %lu", (unsigned long)pInfo->dwPowerCycleCount);
        RPT_LINE("Read Speed      : %d MB/s", pInfo->nReadSpeedMBs);
        RPT_LINE("Predict Failure : %s", pInfo->bPredictFailure ? "YES" : "No");

        /* USB bridge info (if applicable) */
        if (pInfo->bIsUSB) {
            RPT_LINE("USB Bridge VID : 0x%04X", pInfo->wUsbVid);
            RPT_LINE("USB Bridge PID : 0x%04X", pInfo->wUsbPid);
            RPT_LINE("Bridge Vendor  : %s",
                strlen(pInfo->szBridgeVendor) ? pInfo->szBridgeVendor : "(unknown)");
            RPT_LINE("Bridge Product : %s",
                strlen(pInfo->szBridgeProduct) ? pInfo->szBridgeProduct : "(unknown)");
        }

        RPT_LINE("");
        RPT_LINE("---- SMART Attributes / Health Log ----");
        RPT_LINE("");

        if (pInfo->bIsUSB && !pInfo->bSMART_Supported) {
            RPT_LINE("  SMART not available for this USB/External drive.");
            RPT_LINE("  (Bridge chip does not support SAT passthrough.)");
        }
        else if (pInfo->bIsNVMe && !pInfo->bSMART_Supported) {
            RPT_LINE("  NVMe Health Log could not be read.");
            RPT_LINE("  Run as Administrator and click Refresh.");
        }
        else if (pInfo->bIsNVMe && pInfo->bSMART_Supported) {
            ...
            RPT_LINE("  01h  Critical Warning            : 0x%02X", pLog->CriticalWarning);
            RPT_LINE("  02h  Composite Temperature        : %d C (%d K)", nTempC, (int)wTempK);
            RPT_LINE("  03h  Available Spare             : %d %%", (int)pLog->AvailableSpare);
            RPT_LINE("  04h  Available Spare Threshold    : %d %%", (int)pLog->AvailableSpareThreshold);
            RPT_LINE("  05h  Percentage Used (Endurance) : %d %%", (int)pLog->PercentageUsed);
            RPT_LINE("  06h  Data Units Read              : %llu (%.1f GB)",
                qwDataRead, (double)qwDataRead / 2048.0);
            RPT_LINE("  07h  Data Units Written           : %llu (%.1f GB)",
                qwDataWritten, (double)qwDataWritten / 2048.0);
            RPT_LINE("  09h  Power On Hours               : %llu", qwPOH);
            RPT_LINE("  0Ch  Power Cycles                 : %llu", qwPowerCycles);
            RPT_LINE("  10h  Unsafe Shutdowns             : %llu", qwUnsafeSDs);
            RPT_LINE("  11h  Media & Data Integrity Errors: %llu", qwMediaErr);
            RPT_LINE("  12h  Error Log Entries            : %llu", qwErrLog);
            RPT_LINE("  13h  Warning Comp Temp Time       : %lu min", pLog->WarningCompTempTime);
            RPT_LINE("  14h  Critical Comp Temp Time      : %lu min", pLog->CriticalCompTempTime);
        }
        else if (pInfo->bSMART_Supported) {
            RPT_LINE("  ID    Name                              Value  Worst  Threshold  Raw          Status");
            RPT_LINE("  ----  --------------------------------  -----  -----  ---------  -----------  -------");
            int k;
            int nAttrCount = 0;
            for (k = 0; k < 30; k++) {
                SMART_ATTRIBUTE* pAttr = &pInfo->attrData.stAttributes[k];
                ...
                const char* pszStatus = bFailed ? "FAILED"
                    : (bThresh > 0 && pAttr->bAttrValue < bThresh + 10)
                        ? "Warning" : "OK";

                char szAttrName[40];
                _snprintf(szAttrName, sizeof(szAttrName), "%s",
                    GetAttrName(pAttr->bAttrID));

                RPT_LINE("  0x%02X  %-32s  %5d  %5d  %9d  %11lu  %s",
                    pAttr->bAttrID, szAttrName,
                    pAttr->bAttrValue, pAttr->bWorstValue, bThresh,
                    (unsigned long)dwRaw, pszStatus);
                nAttrCount++;
            }
            if (nAttrCount == 0) {
                RPT_LINE("  (No SMART attributes returned - bridge may only expose SCSI Log Sense)");
                if (pInfo->bPredictFailure) {
                    RPT_LINE("  Predictive Failure (SCSI): YES");
                }
            }
        }
        else {
            RPT_LINE("  SMART data not available for this drive.");
        }
        RPT_LINE("");
    }

    RPT_LINE("================================================================");
    RPT_LINE("  End of Report");
    RPT_LINE("================================================================");
    ...
    /* Success message - offer to open folder */
    char szMsg[MAX_PATH + 256];
    _snprintf(szMsg, sizeof(szMsg),
        "Report saved successfully!\n\n%s\n\nOpen folder now?", szFile);
    int nRet = MessageBoxA(hWnd, szMsg, "HDDHealth Monitor - Report Saved",
        MB_YESNO | MB_ICONINFORMATION);
    if (nRet == IDYES) {
        char szCmd[MAX_PATH + 32];
