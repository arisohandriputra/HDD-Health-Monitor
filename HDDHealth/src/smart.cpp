...
#include <stddef.h>
#include <string.h>
#include <stdlib.h>   /* strtol, strtoul, _stricmp, malloc/free, _i64toa  (MSVC v100 needs this explicitly; MinGW pulls it in transitively) */
#include <stdio.h>
#include <ctype.h>
#include <setupapi.h>
#include <devguid.h>
 ...
 *  the link step. On some Visual Studio 2010 installs (typically
 *  VC++ 2010 Express without a separately-installed Windows SDK
 *  v7.0A / v7.1) cfgmgr32.lib is either missing from the SDK or
 *  the library search path is not registered, producing:
 *
 *      error LNK1104: cannot open file 'cfgmgr32.lib'
 *
 *  To make the project build out-of-the-box on every VS2010
 *  installation, the CM_* entry points are now resolved at runtime
 *  via LoadLibraryA("cfgmgr32.dll") + GetProcAddress. The DLL ships
 *  with every version of Windows since 2000, so this is always
 *  safe at runtime; it just removes the link-time .lib dependency.
 * ----------------------------------------------------------------- */

/* Function pointer typedefs matching the cfgmgr32 prototypes. */
typedef DWORD (WINAPI *PFN_CM_Get_Parent_Fn)
    (PDEVINST pdnDevInst, DEVINST dnDevInst, ULONG ulFlags);

typedef DWORD (WINAPI *PFN_CM_Get_Device_ID_Size_Fn)
    (PULONG pulLen, DEVINST dnDevInst, ULONG ulFlags);

typedef DWORD (WINAPI *PFN_CM_Get_Device_IDA_Fn)
    (DEVINST dnDevInst, PCHAR Buffer, ULONG BufferLen, ULONG ulFlags);

/* Globally-cached module handle and function pointers.
 * Loaded once on first use; never released. NULL = not loaded yet. */
static HMODULE                        g_hCfgMgr32       = NULL;
static PFN_CM_Get_Parent_Fn           g_pfn_CM_Get_Parent            = NULL;
static PFN_CM_Get_Device_ID_Size_Fn   g_pfn_CM_Get_Device_ID_Size    = NULL;
static PFN_CM_Get_Device_IDA_Fn       g_pfn_CM_Get_Device_IDA        = NULL;

/* One-time loader. Returns TRUE if at least CM_Get_Parent is available
 * (the other two are always present in the same DLL when present).
 * Thread-safety note: the worst case of a benign race here is that
 * two threads load the DLL twice; LoadLibrary is reference-counted
 * and the second LoadLibrary just bumps the refcount, so this is OK
 * without a mutex. */
static BOOL CfgMgr32_Load(void)
{
    if (g_hCfgMgr32) return TRUE;

    HMODULE h = LoadLibraryA("cfgmgr32.dll");
    if (!h) return FALSE;

    g_pfn_CM_Get_Parent =
        (PFN_CM_Get_Parent_Fn)GetProcAddress(h, "CM_Get_Parent");
    g_pfn_CM_Get_Device_ID_Size =
        (PFN_CM_Get_Device_ID_Size_Fn)GetProcAddress(h, "CM_Get_Device_ID_Size");
    g_pfn_CM_Get_Device_IDA =
        (PFN_CM_Get_Device_IDA_Fn)GetProcAddress(h, "CM_Get_Device_IDA");

    if (!g_pfn_CM_Get_Parent ||
        !g_pfn_CM_Get_Device_ID_Size ||
        !g_pfn_CM_Get_Device_IDA) {
        /* DLL is present but doesn't export the expected entry points -
         * should never happen on any supported Windows version. */
        FreeLibrary(h);
        return FALSE;
    }
...
/* Thin wrappers so the rest of smart.cpp can keep the original
 * CM_* call sites unchanged. Each wrapper returns CR_FAILURE (a
 * non-CR_SUCCESS value) if cfgmgr32 could not be loaded. */

static DWORD Wrap_CM_Get_Parent(PDEVINST pdnDevInst, DEVINST dnDevInst, ULONG ulFlags)
{
    if (!CfgMgr32_Load()) return 0x0000001FL /* CR_FAILURE */;
    return g_pfn_CM_Get_Parent(pdnDevInst, dnDevInst, ulFlags);
}

static DWORD Wrap_CM_Get_Device_ID_Size(PULONG pulLen, DEVINST dnDevInst, ULONG ulFlags)
{
    if (!CfgMgr32_Load()) { if (pulLen) *pulLen = 0; return 0x0000001FL; }
    return g_pfn_CM_Get_Device_ID_Size(pulLen, dnDevInst, ulFlags);
}

static DWORD Wrap_CM_Get_Device_IDA(DEVINST dnDevInst, PCHAR Buffer, ULONG BufferLen, ULONG ulFlags)
{
    if (!CfgMgr32_Load()) { if (Buffer && BufferLen) Buffer[0] = '\0'; return 0x0000001FL; }
    return g_pfn_CM_Get_Device_IDA(dnDevInst, Buffer, BufferLen, ulFlags);
}

/* Macro redirection: any CM_* call in the rest of this file is
 * silently rewritten to our wrapped version above. This is the only
 * way to keep the original source-code call sites byte-identical to
 * upstream while removing the link-time cfgmgr32.lib dependency. */
#define CM_Get_Parent          Wrap_CM_Get_Parent
#define CM_Get_Device_ID_Size  Wrap_CM_Get_Device_ID_Size
#define CM_Get_Device_IDA      Wrap_CM_Get_Device_IDA
